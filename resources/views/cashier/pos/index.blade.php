    ...
    <nav class="top-nav">
        ...
        <div class="collapse d-lg-flex top-nav-menu flex-grow-1 ms-lg-3" id="topNavMenu">
            <div class="nav-menu-inner d-flex w-100 gap-2">
                ...
                <a href="{{ route('cashier.pos.page') }}" class="nav-pill active"><i class="bi bi-calculator"></i>
                    POS</a>
                <a href="{{ route('cashier.orders.page') }}" class="nav-pill"><i class="bi bi-list-ul"></i> Orders</a>
                <a href="{{ route('cashier.pending_orders.page') }}" class="nav-pill">
...
════════════════════════════ -->
            ...
            <div class="grid-scroll">
                <div class="products-grid">
                    ...
                    @foreach ($products as $product)
                        <form action="{{ route('cashier.pos.cart.add') }}" method="POST" class="pcard-form m-0 p-0"
                    ...
                    @foreach ($cartItems as $item)
                        <div class="order-item">
                            ...
                            <div class="oi-controls">
                                <form action="{{ route('cashier.pos.cart.update', $item->id) }}" method="POST"
                            ...
                            <form action="{{ route('cashier.pos.cart.delete', $item->id) }}" method="POST"
                                class="m-0 p-0">
                                @csrf
                                @method('DELETE')
    ...
    @include('cashier.pos.modals.custom_item')
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            if (qtyModal) {
                qtyModal.addEventListener('show.bs.modal', function(event) {
                    ...
                    form.action = `/cashier/pos/cart/${cartId}/update`;
