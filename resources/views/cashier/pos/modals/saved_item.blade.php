<form id="saveOrderForm" action="{{ route('cashier.pos.cart.save') }}" method="POST" style="display: none;">
    @csrf
</form>
                                ...
                                <div id="collapse-{{ $reference }}" class="accordion-collapse collapse"
                                    ...
                                    <div class="accordion-body">
                                                ...
                                                <tbody>
                                                    ...
                                                    @foreach ($items as $item)
                                                        @php
                                                            ...
                                                            if (
                                                                $item->product_id &&
                                                                !empty($item->whole_sale_qty) &&
                                                                $item->whole_sale_qty > 0 &&
                                                                $item->quantity >= $item->whole_sale_qty
                                                            ) {
                                                                $wholesale_bundles = floor(
                                                                    $item->quantity / $item->whole_sale_qty,
                                                                );
                                                                $regular_items = fmod(
                                                                    $item->quantity,
                                                                ...
                                                                $subtotal =
                                                                    $wholesale_bundles * $item->whole_sale_price +
                                                                    $regular_items * $item->selling_price;
                                                                $isWholesale = true;
                                                            } else {
                                                                    ...
                                                                    <span>{{ $name }}
                                                                        @if ($isWholesale)
                                                                            <br><small class="badge bg-success mt-1"
                                                                                style="font-size: 0.65em; padding: 2px 4px;">Wholesale
                                                                                Applied</small>
                                                                        @endif
                                                                    </span>
                                                                </div>
                                        ...
                                        <div class="d-flex justify-content-end gap-2 mt-3">
                                            <form action="{{ route('cashier.pos.saved.delete', $reference) }}"
                                            ...
                                            <form action="{{ route('cashier.pos.saved.load', $reference) }}"
                                                method="POST" class="m-0">
                                                @csrf
