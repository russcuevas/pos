...
<body class="pos-page">
    ...
    <nav class="top-nav">
        <a href="{{ route('cashier.pos.page') }}" class="nav-brand">
        ...
        <div class="collapse d-lg-flex top-nav-menu flex-grow-1 ms-lg-3" id="topNavMenu">
            <div class="nav-menu-inner d-flex w-100 gap-2">
                ...
                <a href="{{ route('cashier.pos.page') }}" class="nav-pill"><i class="bi bi-calculator"></i> POS</a>
                <a href="{{ route('cashier.orders.page') }}" class="nav-pill"><i class="bi bi-list-ul"></i> Orders</a>
        ...
        <div class="orders-container">
            ...
            @else
                @foreach ($orders as $order)
                    @include('cashier.pending_orders.order_card_partial', ['order' => $order])
                @endforeach
            @endif
        </div>
    </div>
    ...
    <script>
        ...
        document.addEventListener('click', async (e) => {
            ...
            if (e.target.closest('.btn-send-chat')) {
                ...
                const response = await fetch("{{ route('cashier.pending_orders.send_chat') }}", {
                    method: 'POST',
                    headers: {
            ...
            if (e.target.closest('.btn-order-qty-adj')) {
                ...
                const response = await fetch("{{ route('cashier.pending_orders.update_qty') }}", {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'X-CSRF-TOKEN': csrfToken
            ...
            if (e.target.closest('.btn-action-start')) {
                ...
                const response = await fetch("{{ route('cashier.pending_orders.start_preparing') }}", {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'X-CSRF-TOKEN': csrfToken
            ...
            if (e.target.closest('.btn-action-ready')) {
                ...
                const response = await fetch("{{ route('cashier.pending_orders.mark_ready') }}", {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'X-CSRF-TOKEN': csrfToken
            ...
            if (e.target.closest('.btn-cancel-order')) {
                ...
                if (result.isConfirmed) {
                    const response = await fetch("{{ route('cashier.pending_orders.cancel') }}", {
                        method: 'POST',
                        headers: {
                            'Content-Type': 'application/json',
                            'X-CSRF-TOKEN': csrfToken
            ...
            if (e.target.closest('.modal-product-btn')) {
                ...
                const response = await fetch("{{ route('cashier.pending_orders.add_item') }}", {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'X-CSRF-TOKEN': csrfToken
        ...
        document.getElementById('pendingCheckoutForm')?.addEventListener('submit', async function(e) {
            ...
            const response = await fetch("{{ route('cashier.pending_orders.checkout') }}", {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'X-CSRF-TOKEN': csrfToken
        ...
        async function checkNewOrders() {
            try {
                const response = await fetch("{{ route('cashier.pending_orders.check') }}");
